...
function encriptar() {
    if (textarea.value !== "") {
        const texto = encriptarMensaje(filterSpecialCharacters(textarea.value));
...
function desencriptar() {
    if (textarea.value !== "") {
        const texto = desencriptarMensaje(filterSpecialCharacters(textarea.value));
        mensajeCodificado.value = texto;
        textarea.value = "";
        imagenHacker.style.display = "none";
...
async function pegar() {
    ...
    const filteredText = filterSpecialCharacters(textoCopiado);
    if (filteredText !== textoCopiado) {
        // Si se han eliminado caracteres especiales, mostramos la alerta
        alert("Se han eliminado caracteres especiales del texto pegado.");
    }
    const textarea = document.querySelector('.textarea');
...
function filterSpecialCharacters(inputString) {
    // Utilizamos una expresión regular para eliminar los caracteres especiales
    const filteredString = inputString.replace(/[^a-zA-Z0-9\s]/g, '');
    if (filteredString !== inputString) {
        // Si se han eliminado caracteres especiales, mostramos la alerta
        alert("Se han eliminado caracteres especiales del texto ingresado.");
    }
